...
function headlineFor(template?: string) {
  return TEMPLATE_HEADLINES[template || "classic"] || "خبر عاجل!!!";
}
...
<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1350" viewBox="0 0 1080 1350">
  ...
  <text x="92" y="74" font-family="${englishFont}" font-size="31" font-weight="900" fill="#120d09">Wedding invitation</text>
  <text x="540" y="76" font-family="${englishFont}" font-size="38" font-weight="900" text-anchor="middle" fill="#4c2b11">♥</text>
  <text x="988" y="74" font-family="${englishFont}" font-size="32" font-weight="900" text-anchor="end" fill="#120d09">${escapeXml(date.headerDate)}</text>
  <rect x="112" y="92" width="856" height="5" fill="#000"/>
  <text x="540" y="188" direction="rtl" unicode-bidi="bidi-override" font-family="${arabicFont}" font-size="98" font-weight="900" text-anchor="middle" fill="#e73539">${escapeXml(headline)}</text>
  <rect x="112" y="254" width="856" height="5" fill="#000"/><rect x="112" y="267" width="856" height="5" fill="#000"/>
  <text x="540" y="383" direction="rtl" unicode-bidi="bidi-override" font-family="${arabicFont}" font-size="76" font-weight="900" text-anchor="middle" fill="#4b2b10">${escapeXml(couple)}</text>
  ...
  <text x="245" y="1062" font-family="${englishFont}" font-size="70" font-weight="900" text-anchor="middle" fill="#fff">${escapeXml(date.day)}</text>
  <text x="245" y="1158" font-family="${englishFont}" font-size="48" text-anchor="middle" fill="#fff">${escapeXml(date.month)}</text>
  <text x="245" y="1248" font-family="${englishFont}" font-size="56" font-weight="900" text-anchor="middle" fill="#fff">${escapeXml(date.year)}</text>
  <rect x="480" y="954" width="3" height="392" fill="#000"/>
  <text x="744" y="1008" font-family="${englishFont}" font-size="58" font-weight="900" text-anchor="middle" fill="#000">Save the date</text>
  ...
  <text x="802" y="1142" font-family="${englishFont}" font-size="54" text-anchor="middle" fill="#000">${escapeXml(date.month)}</text>
  <text x="802" y="1194" font-family="${arabicFont}" font-size="28" font-weight="900" text-anchor="middle" fill="#000">${escapeXml(venue)}</text>
  <text x="802" y="1256" font-family="${englishFont}" font-size="34" font-weight="900" text-anchor="middle" fill="#000">At ${escapeXml(time)}</text>
  <image href="${qrDataUrl}" x="612" y="1190" width="116" height="116"/>
  <text x="670" y="1320" direction="rtl" unicode-bidi="bidi-override" font-family="${arabicFont}" font-size="15" font-weight="900" text-anchor="middle" fill="#20140d">امسح الكود لمشاهدة الدعوة كاملة</text>
...
export async function generateSharePosterPng(input: ServerSharePosterInput) {
  ...
  try {
    ...
    console.log(`[Share Poster] Generated ${saved.url} (${png.length} bytes, ${Date.now() - startedAt}ms) for template "${input.selectedShareTemplate || "classic"}" - ${input.groomName}/${input.brideName}`);
    return saved.url;
  } catch (error) {
    console.error(`[Share Poster] Failed to generate poster for ${input.groomName}/${input.brideName}:`, error instanceof Error ? error.message : String(error));
    throw error;
  }
}
