...
async function collectDatabaseReferences(references: Map<string, MediaUsageDetail[]>) {
  ...
  const [invitations, orders, templates] = await Promise.all([
    prisma.invitation.findMany({ select: { code: true, groomName: true, brideName: true, heroPhoto: true, gallery: true, musicUrl: true, photographer: true, qrCodeUrl: true, postImageUrl: true, postImageOgUrl: true } }).catch(() => []),
