...
function isCleanupAction(value: string): value is StorageCleanupAction {
  return value === "orphans" || value === "duplicates" || value === "original-images" || value === "music-unused" || value === "old-backups" || value === "all";
}
...
export async function POST(request: NextRequest) {
  ...
  queueGitHubSync(`Storage cleanup (${action}) deleted ${result.deletedFiles.length} file(s) and ${result.deletedBackups.length} backup(s). Backup: ${result.backupFileName}.`, { createSnapshot: true });
  const deletedImages = result.deletedFiles.filter((file) => file.kind === "image");
  if (result.deletedFiles.length || result.deletedBackups.length) {
    await recordAuditLog({
      actor: await getAuditActorFromAdminRequest(request),
      action: "media.image.delete",
      entity: { type: "Media", id: "storage-cleanup", label: `${result.deletedFiles.length} file(s), ${result.deletedBackups.length} backup(s)` },
      oldValues: { files: result.deletedFiles, backups: result.deletedBackups },
      newValues: { deleted: true, action, filesCount: result.deletedFiles.length, backupsCount: result.deletedBackups.length },
      metadata: {
