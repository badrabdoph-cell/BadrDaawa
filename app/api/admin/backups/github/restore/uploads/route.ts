...
import { restoreUploadsFromGitHub, logRestoreAttempt } from "@/lib/backups";
...
export async function POST(request: NextRequest) {
  ...
  if (operationId) updateOperation(operationId, { progress: 5, step: "بدء استعادة الملفات", status: "in_progress" });
  ...
  try {
    ...
    if (operationId) {
      if (result.ok) {
        ...
        failOperation(operationId, result.error || "فشلت استعادة الملفات بدون تفاصيل");
      }
    }
