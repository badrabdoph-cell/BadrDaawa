...
export async function POST(request: NextRequest) {
  ...
  if (operationId) updateOperation(operationId, { progress: 5, step: "بدء إنشاء النسخة الكاملة", status: "in_progress" });

  try {
    if (operationId) updateOperation(operationId, { progress: 15, step: "قراءة بيانات قاعدة البيانات" });
    ...
    const result = await uploadFullBackupToGitHub(
      manifest, files,
